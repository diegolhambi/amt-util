...
/** Sobrescreve config com flags CLI. */
...
function getSessionFilePath() {
  const hash = md5(`${config.host}:${config.port}:${config.user}`);
  return join(homedir(), '.cache', 'amt-util', `session-${hash}.json`);
}
...
async function request(method, path, data = null) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Reusa authChallenge da sessão imediatamente se disponível
      if (authChallenge) {
      ...
      // Se não autenticado (ex: nonce expirou), limpa o cache, atualiza challenge e tenta novamente
      if (res.status === 401) {
        clearSession();
      ...
      // Persiste a sessão para os próximos comandos
