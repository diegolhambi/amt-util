...
export const CATEGORIES = Object.keys(HW_PAGES);

export async function run(category, { json } = {}) {
  if (category && !CATEGORIES.includes(category)) {
    process.stderr.write(
      `amt-util: unknown category "${category}" — valid: ${CATEGORIES.join(', ')}\n`
    );
    process.exit(2);
  }
